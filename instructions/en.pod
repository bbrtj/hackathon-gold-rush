...
It is recommended to execute the orders synchronously by waiting for a response
before executing the next order. Order responses contain no identifiers, so
there will be no way to tell from which order they come otherwise.
